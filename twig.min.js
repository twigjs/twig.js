var twig=function(a){var b=a.id;if(a.debug!==void 0)Twig.debug=a.debug;if(a.data!==void 0)return new Twig.Template({data:a.data,id:b});else if(a.ref!==void 0){if(a.id!==void 0)throw Error("Both ref and id cannot be set on a twig.js template.");return Twig.Templates.load(a.ref)}else if(a.href!==void 0)return Twig.Templates.loadRemote(a.href,{id:b,precompiled:a.precompiled},a.async,a.load)};twig.extendFilter=function(a,b){Twig.extendFilter(a,b)};twig.extendTest=function(a,b){Twig.extendFilter(a,b)};
twig.extendTag=function(a){Twig.logic.extend(a)};twig.compile=function(a,b){var c=new Twig.Template({data:a,id:b.filename});return function(a){return c.render(a)}};
...
{output:"output",logic:"logic",comment:"comment",raw:"raw"};a.token.definitions={output:{type:a.token.type.output,open:"{{",close:"}}"},logic:{type:a.token.type.logic,open:"{%",close:"%}"},comment:{type:a.token.type.comment,open:"{#",close:"#}"}};a.token.strings=['"',"'"];a.token.findStart=function(c){var b={position:null,def:null},e,f,g;for(e in a.token.definitions)if(a.token.definitions.hasOwnProperty(e)&&(f=a.token.definitions[e],g=c.indexOf(f.open),a.log.trace("Twig.token.findStart: ","Searching for ",
f.open," found at ",g),g>=0&&(b.position===null||g<b.position)))b.position=g,b.def=f;return b};a.token.findEnd=function(c,b,e){for(var f=null,g=false,h=0,j=null,k=null,i=null,l=j=null,j=null,m,n;!g;){k=j=null;i=c.indexOf(b.close,h);if(i>=0)f=i,g=true;else throw new a.Error("Unable to find closing bracket '"+b.close+"' opened near template position "+e);n=a.token.strings.length;for(m=0;m<n;m+=1)if(l=c.indexOf(a.token.strings[m],h),l>0&&l<i&&(j===null||l<j))j=l,k=a.token.strings[m];if(j!==null){j+=
1;f=null;for(g=false;;){j=c.indexOf(k,j);if(j<0)throw"Unclosed string in template";if(c.substr(j-1,1)!=="\\"){h=j+1;break}else j+=1}}}return f};a.tokenize=function(c){for(var b=[],e=0,f=null,g=null;c.length>0;)f=a.token.findStart(c),a.log.trace("Twig.tokenize: ","Found token: ",f),f.position!==null?(f.position>0&&b.push({type:a.token.type.raw,value:c.substring(0,f.position)}),c=c.substr(f.position+f.def.open.length),e+=f.position+f.def.open.length,g=a.token.findEnd(c,f.def,e),a.log.trace("Twig.tokenize: ",
"Token ends at ",g),b.push({type:f.def.type,value:c.substring(0,g).trim()}),c=c.substr(g+f.def.close.length),e+=g+f.def.close.length):(b.push({type:a.token.type.raw,value:c}),c="");return b};a.compile=function(c){for(var b=[],e=[],f=[],g=null,h=g=g=null,j=null,k=h=null,i=null,l=null;c.length>0;){g=c.shift();a.log.trace("Compiling token ",g);switch(g.type){case a.token.type.raw:e.length>0?f.push(g):b.push(g);break;case a.token.type.logic:g=a.logic.compile.apply(this,[g]);k=g.type;i=a.logic.handler[k].open;
l=a.logic.handler[k].next;a.log.trace("Twig.compile: ","Compiled logic token to ",g," next is: ",l," open is : ",i);if(i!==void 0&&!i){h=e.pop();j=a.logic.handler[h.type];if(j.next.indexOf(k)<0)throw Error(k+" not expected after a "+h.type);h.output=h.output||[];h.output=h.output.concat(f);f=[];h={type:a.token.type.logic,token:h};e.length>0?f.push(h):b.push(h)}if(l!==void 0&&l.length>0){a.log.trace("Twig.compile: ","Pushing ",g," to logic stack.");if(e.length>0)h=e.pop(),h.output=h.output||[],h.output=
h.output.concat(f),e.push(h);e.push(g)}else i!==void 0&&i&&(h={type:a.token.type.logic,token:g},e.length>0?f.push(h):b.push(h));break;case a.token.type.output:a.expression.compile.apply(this,[g]),e.length>0?f.push(g):b.push(g)}a.log.trace("Twig.compile: "," Output: ",b," Logic Stack: ",e," Pending Output: ",f)}if(e.length>0)throw g=e.pop(),Error("Unable to find an end tag for "+g.type+", expecting one of "+g.next.join(", "));return b};a.parse=function(c,b){var e=[],f=true,g=this,b=b||{};c.forEach(function(c){a.log.debug("Twig.parse: ",
"Parsing token: ",c);switch(c.type){case a.token.type.raw:e.push(c.value);break;case a.token.type.logic:c=a.logic.parse.apply(g,[c.token,b,f]);if(c.chain!==void 0)f=c.chain;if(c.context!==void 0)b=c.context;c.output!==void 0&&e.push(c.output);break;case a.token.type.output:e.push(a.expression.parse.apply(g,[c.stack,b]))}});return e.join("")};a.prepare=function(c){a.log.debug("Twig.prepare: ","Tokenizing ",c);c=a.tokenize.apply(this,[c]);a.log.debug("Twig.prepare: ","Compiling ",c);c=a.compile.apply(this,
[c]);a.log.debug("Twig.prepare: ","Compiled ",c);return c};a.Templates={registry:{}};a.Templates.save=function(c){if(c.id===void 0)throw new a.Error("Unable to save template with no id");a.Templates.registry[c.id]=c};a.Templates.load=function(c){return!a.Templates.registry.hasOwnProperty(c)?null:a.Templates.registry[c]};a.Templates.loadRemote=function(c,b,e,f){var g=b.id,h=b.blocks,j=b.precompiled,k=null;g===void 0&&(g=c);if(a.Templates.registry.hasOwnProperty(g))return a.Templates.registry.hasOwnProperty(g);
if(typeof XMLHttpRequest=="undefined")throw Error("Unsupported platform: Unable to do remote requests because there is no XMLHTTPRequest implementation");var i=new XMLHttpRequest;i.onreadystatechange=function(){var b=null;if(i.readyState==4)a.log.debug("Got template ",i.responseText),j===true?(b=JSON.parse(i.responseText),k=new a.Template({data:b,blocks:h,id:g})):(k=new a.Template({data:i.responseText,blocks:h,id:g}),k.url=c),f&&f(k)};i.open("GET",c,e===true);i.send();if(e===false)return k};a.Template=
function(c){var d=c.data,e=c.id,c=c.blocks,f;this.id=e;this.blocks=c||{};this.extend=null;this.tokens=b("String",d)?a.prepare.apply(this,[d]):d;this.render=function(c){var b=a.parse.apply(this,[this.tokens,c]);console.log(this);if(this.extend){var d=this.extend,b=this.url.split("/"),e=[];b.pop();for(b=b.concat(d.split("/"));b.length>0;)d=b.shift(),d!="."&&(d==".."&&e.length>0&&e[e.length-1]!=".."?e.pop():e.push(d));f=e.join("/");console.log("Loading ",f);this.parent=a.Templates.loadRemote(f,{id:f,
blocks:this.blocks},false);this.parent.blocks=this.blocks;return this.parent.render(c)}return b};e!==void 0&&a.Templates.save(this)};return a}(Twig||{});
...
Twig=function(a){a.logic={};a.logic.type={if_:"Twig.logic.type.if",endif:"Twig.logic.type.endif",for_:"Twig.logic.type.for",endfor:"Twig.logic.type.endfor",else_:"Twig.logic.type.else",elseif:"Twig.logic.type.elseif",set:"Twig.logic.type.set",filter:"Twig.logic.type.filter",endfilter:"Twig.logic.type.endfilter",block:"Twig.logic.type.block",endblock:"Twig.logic.type.endblock",extends_:"Twig.logic.type.extends"};a.logic.definitions=[{type:a.logic.type.if_,regex:/^if\s+([^\s].+)$/,next:[a.logic.type.else_,
a.logic.type.elseif,a.logic.type.endif],open:true,compile:function(b){b.stack=a.expression.compile.apply(this,[{type:a.expression.type.expression,value:b.match[1]}]).stack;delete b.match;return b},parse:function(b,c,d){var e="",f=a.expression.parse.apply(this,[b.stack,c]),d=true;f===true&&(d=false,e=a.parse.apply(this,[b.output,c]));return{chain:d,output:e}}},{type:a.logic.type.elseif,regex:/^elseif\s+([^\s].*)$/,next:[a.logic.type.else_,a.logic.type.endif],open:false,compile:function(b){b.stack=
...
next:[],open:true,compile:function(b){var c=b.match[1].trim();delete b.match;b.stack=a.expression.compile.apply(this,[{type:a.expression.type.expression,value:c}]).stack;return b},parse:function(b,c,d){this.extend=a.expression.parse.apply(this,[b.stack,c]);return{chain:d,output:""}}}];a.logic.handler={};a.logic.extendType=function(b,c){a.logic.type[b]=c||"Twig.logic.type"+b};for(a.logic.extend=function(b){if(!b.type)throw new a.Error("Unable to extend logic definition. No type provided for "+b);if(a.logic.type[b.type])throw new a.Error("Unable to extend logic definitions. Type "+
b.type+" is already defined.");else a.logic.extendType(b.type);a.logic.handler[b.type]=b};a.logic.definitions.length>0;)a.logic.extend(a.logic.definitions.shift());a.logic.compile=function(b){var b=b.value.trim(),b=a.logic.tokenize.apply(this,[b]),c=a.logic.handler[b.type];c.compile&&(b=c.compile.apply(this,[b]),a.log.trace("Twig.logic.compile: ","Compiled logic token to ",b));return b};a.logic.tokenize=function(b){var c={},d=null,e=null,f=null,g=null,f=f=null,b=b.trim();for(d in a.logic.handler)if(a.logic.handler.hasOwnProperty(d)){e=
a.logic.handler[d].type;f=a.logic.handler[d].regex;g=[];for(f instanceof Array?g=f:g.push(f);g.length>0;)if(f=g.shift(),f=f.exec(b.trim()),f!==null)return c.type=e,c.match=f,a.log.trace("Twig.logic.tokenize: ","Matched a ",e," regular expression of ",f),c}throw new a.Error("Unable to parse '"+b.trim()+"'");};a.logic.parse=function(b,c,d){var e="",f,c=c||{};a.log.debug("Twig.logic.parse: ","Parsing logic token ",b);f=a.logic.handler[b.type];f.parse&&(e=f.parse.apply(this,[b,c,d]));return e};return a}(Twig||
{});
Twig=function(a){a.expression={};a.expression.type={comma:"Twig.expression.type.comma",expression:"Twig.expression.type.expression",operator:"Twig.expression.type.operator",string:"Twig.expression.type.string",array:{start:"Twig.expression.type.array.start",end:"Twig.expression.type.array.end"},object:{start:"Twig.expression.type.object.start",end:"Twig.expression.type.object.end"},parameter:{start:"Twig.expression.type.parameter.start",end:"Twig.expression.type.parameter.end"},key:{period:"Twig.expression.type.key.period",brackets:"Twig.expression.type.key.brackets"},
filter:"Twig.expression.type.filter",variable:"Twig.expression.type.variable",number:"Twig.expression.type.number",setkey:"Twig.expression.type.setkey",test:"Twig.expression.type.test"};a.expression.set={operations:[a.expression.type.filter,a.expression.type.operator,a.expression.type.array.end,a.expression.type.object.end,a.expression.type.parameter.end,a.expression.type.comma,a.expression.type.setkey,a.expression.type.test],expressions:[a.expression.type.expression,a.expression.type.string,a.expression.type.variable,
a.expression.type.number,a.expression.type.array.start,a.expression.type.object.start]};a.expression.fn={compile:{push:function(a,c,d){d.push(a)}},parse:{push:function(a,c){c.push(a)},push_value:function(a,c){c.push(a.value)}}};a.expression.definitions=[{type:a.expression.type.test,regex:/^is\s+(not)?\s*([a-zA-Z_][a-zA-Z0-9_]*)/,next:a.expression.set.operations.concat([a.expression.type.parameter.start]),compile:function(a,c,d){a.filter=a.match[2];a.modifier=a.match[1];delete a.match;delete a.value;
d.push(a)},parse:function(b,c,d){var e=c.pop(),d=b.params&&a.expression.parse.apply(this,[b.params,d]),e=a.test(b.filter,e,d);b.modifier=="not"?c.push(!e):c.push(e)}},{type:a.expression.type.setkey,regex:/^\:/,next:a.expression.set.expressions,compile:function(b,c,d){c=d.pop();if(c.type!==a.expression.type.string)throw new a.Error("Unexpected object key: "+c);b.key=c.value;d.push(b)},parse:a.expression.fn.parse.push},{type:a.expression.type.comma,regex:/^,/,next:a.expression.set.expressions,compile:function(a,
c,d){for(;c.length>0;)d.push(c.pop());d.push(a)}},{type:a.expression.type.expression,regex:/^\(([^\)]+)\)/,next:a.expression.set.operations,compile:function(b,c,d){b.value=b.match[1];for(b=a.expression.compile(b).stack;b.length>0;)d.push(b.shift())}},{type:a.expression.type.operator,regex:/(^[\+\*\/\-\^~%]|^[<>!]=?|^==|^\|\||^&&)/,next:a.expression.set.expressions,compile:function(b,c,d){b=a.expression.operator.lookup(b.value,b);for(a.log.trace("Twig.expression.compile: ","Operator: ",b);c.length>
0&&(b.associativity===a.expression.operator.leftToRight&&b.precidence>=c[c.length-1].precidence||b.associativity===a.expression.operator.rightToLeft&&b.precidence>c[c.length-1].precidence);)d.push(c.pop());c.push(b)},parse:function(b,c){a.expression.operator.parse(b.value,c)}},{type:a.expression.type.string,regex:/^(["'])(?:(?=(\\?))\2.)*?\1/,next:a.expression.set.operations,compile:function(b,c,d){c=b.value;c=c.substring(0,1)==='"'?c.replace('\\"','"'):c.replace("\\'","'");b.value=c.substring(1,
c.length-1);a.log.trace("Twig.expression.compile: ","String value: ",b.value);d.push(b)},parse:a.expression.fn.parse.push_value},{type:a.expression.type.parameter.start,regex:/^\(/,next:a.expression.set.expressions.concat([a.expression.type.parameter.end]),compile:a.expression.fn.compile.push,parse:a.expression.fn.parse.push},{type:a.expression.type.parameter.end,regex:/^\)/,next:a.expression.set.operations,compile:function(b,c,d){for(;c.length>0;)d.push(c.pop());for(c=[];b.type!==a.expression.type.parameter.start;)c.unshift(b),
b=d.pop();c.unshift(b);b=d.pop();if(b.type!==a.expression.type.filter&&b.type!==a.expression.type.test)throw new a.Error("Expected filter before parameters, got "+b.type);b.params=c;d.push(b)},parse:function(b,c){for(var d=[],e=false,f=null;c.length>0;){f=c.pop();if(f.type&&f.type==a.expression.type.parameter.start){e=true;break}d.unshift(f)}if(!e)throw new a.Error("Expected end of parameter set.");c.push(d)}},{type:a.expression.type.array.start,regex:/^\[/,next:a.expression.set.expressions.concat([a.expression.type.array.end]),
compile:a.expression.fn.compile.push,parse:a.expression.fn.parse.push},{type:a.expression.type.array.end,regex:/^\]/,next:a.expression.set.operations.concat([a.expression.type.key.period,a.expression.type.key.brackets]),compile:function(a,c,d){for(;c.length>0;)d.push(c.pop());d.push(a)},parse:function(b,c){for(var d=[],e=false,f=null;c.length>0;){f=c.pop();if(f.type&&f.type==a.expression.type.array.start){e=true;break}d.unshift(f)}if(!e)throw new a.Error("Expected end of array.");c.push(d)}},{type:a.expression.type.object.start,
regex:/^\{/,next:a.expression.set.expressions.concat([a.expression.type.object.end]),compile:a.expression.fn.compile.push,parse:a.expression.fn.parse.push},{type:a.expression.type.object.end,regex:/^\}/,next:a.expression.set.operations.concat([a.expression.type.key.period,a.expression.type.key.brackets]),compile:function(a,c,d){for(;c.length>0;)d.push(c.pop());d.push(a)},parse:function(b,c){for(var d={},e=false,f=null,g=null;c.length>0;){f=c.pop();if(f.type&&f.type===a.expression.type.object.start){e=
true;break}if(f.type&&f.type===a.expression.type.setkey){if(g===null)throw new a.Error("Expected value for key "+f.key+" in object definition. Got "+f);d[f.key]=g;if(d._keys===void 0)d._keys=[];d._keys.unshift(f.key);g=null}else g=f}if(!e)throw new a.Error("Unexpected end of object.");c.push(d)}},{type:a.expression.type.filter,regex:/^\|[a-zA-Z_][a-zA-Z0-9_\-]*/,next:a.expression.set.operations.concat([a.expression.type.key.period,a.expression.type.key.brackets,a.expression.type.parameter.start]),
compile:function(a,c,d){a.value=a.value.substr(1);d.push(a)},parse:function(b,c,d){var e=c.pop(),d=b.params&&a.expression.parse.apply(this,[b.params,d]);c.push(a.filter(b.value,e,d))}},{type:a.expression.type.variable,regex:/^[a-zA-Z_][a-zA-Z0-9_]*/,next:a.expression.set.operations.concat([a.expression.type.key.period,a.expression.type.key.brackets]),compile:a.expression.fn.compile.push,parse:function(b,c,d){b=a.expression.resolve(b.value,d);c.push(b)}},{type:a.expression.type.key.period,regex:/^\.([a-zA-Z_][a-zA-Z0-9_]*)/,
next:a.expression.set.operations.concat([a.expression.type.key.period,a.expression.type.key.brackets]),compile:function(a,c,d){a.key=a.match[1];delete a.match;delete a.value;d.push(a)},parse:function(b,c){var d=b.key,e=c.pop();if(e===null||e===void 0)throw new a.Error("Can't access a key "+d+" on an undefined object.");c.push(e[d])}},{type:a.expression.type.key.brackets,regex:/^\[([^\]]*)\]/,next:a.expression.set.operations.concat([a.expression.type.key.period,a.expression.type.key.brackets]),compile:function(b,
c,d){c=b.match[1];delete b.value;delete b.match;b.stack=a.expression.compile({value:c}).stack;d.push(b)},parse:function(b,c,d){b=a.expression.parse.apply(this,[b.stack,d]);d=c.pop();if(!d.hasOwnProperty(b))throw new a.Error("Model doesn't provide the key "+b);c.push(d[b])}},{type:a.expression.type.number,regex:/^\-?\d*\.?\d+/,next:a.expression.set.operations,compile:a.expression.fn.compile.push,parse:a.expression.fn.parse.push_value}];a.expression.resolve=function(a,c){var d=c[a];return typeof d==
"function"?d.apply(c,[c]):d};a.expression.handler={};a.expression.extendType=function(b){a.expression.type[b]="Twig.expression.type."+b};for(a.expression.extend=function(b){if(!b.type)throw new a.Error("Unable to extend logic definition. No type provided for "+b);a.expression.handler[b.type]=b};a.expression.definitions.length>0;)a.expression.extend(a.expression.definitions.shift());a.expression.tokenize=function(b){var c=[],d=0,e=null,f,g,h,j,k,i=[],l;l=function(){var b=Array.prototype.slice.apply(arguments);
b.pop();b.pop();a.log.trace("Twig.expression.tokenize","Matched a ",f," regular expression of ",b);if(e&&e.indexOf(f)<0)return i.push(f+" cannot follow a "+c[c.length-1].type+" at template:"+d+" near '"+b[0].substring(0,20)+"...'"),b[0];i=[];c.push({type:f,value:b[0],match:b});k=true;e=j;d+=b[0].length;return""};for(a.log.debug("Twig.expression.tokenize","Tokenizing expression ",b);b.length>0;){b=b.trim();for(f in a.expression.handler)if(a.expression.handler.hasOwnProperty(f)){j=a.expression.handler[f].next;
g=a.expression.handler[f].regex;a.log.trace("Checking type ",f," on ",b);h=g instanceof Array?g:[g];for(k=false;h.length>0;)g=h.pop(),b=b.replace(g,l);if(k)break}if(!k)if(i.length>0)throw new a.Error(i.join(" OR "));else throw new a.Error("Unable to parse '"+b+"' at template position"+d);}a.log.trace("Twig.expression.tokenize","Tokenized to ",c);return c};a.expression.compile=function(b){var c=b.value,d=a.expression.tokenize(c),e=null,f=[],g=[],h=null;a.log.trace("Twig.expression.compile: ","Compiling ",
c);for(a.log.trace("Twig.expression.compile: ","Tokens tokenized to ",d);d.length>0;)e=d.shift(),h=a.expression.handler[e.type],h.compile&&h.compile(e,g,f);for(;g.length>0;)f.push(g.pop());a.log.trace("Twig.expression.compile: ","Stack is",f);b.stack=f;delete b.value;return b};a.expression.parse=function(b,c){var d=this;b instanceof Array||(b=[b]);var e=[],f=null;b.forEach(function(b){f=a.expression.handler[b.type];f.parse&&f.parse.apply(d,[b,e,c])});return e.pop()};return a}(Twig||{});
