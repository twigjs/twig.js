...
e,h,g;for(e in a.token.definitions)if(a.token.definitions.hasOwnProperty(e)&&(h=a.token.definitions[e],g=c.indexOf(h.open),a.log.trace("Twig.token.findStart: ","Searching for ",h.open," found at ",g),g>=0&&(b.position===null||g<b.position)))b.position=g,b.def=h;return b};a.token.findEnd=function(c,b,e){for(var h=null,g=false,i=0,j=null,l=null,k=null,m=j=null,j=null,n,o;!g;){l=j=null;k=c.indexOf(b.close,i);if(k>=0)h=k,g=true;else throw new a.Error("Unable to find closing bracket '"+b.close+"' opened near template position "+
e);o=a.token.strings.length;for(n=0;n<o;n+=1)if(m=c.indexOf(a.token.strings[n],i),m>0&&m<k&&(j===null||m<j))j=m,l=a.token.strings[n];if(j!==null){j+=1;h=null;for(g=false;;){j=c.indexOf(l,j);if(j<0)throw"Unclosed string in template";if(c.substr(j-1,1)!=="\\"){i=j+1;break}else j+=1}}}return h};a.tokenize=function(c){for(var b=[],e=0,h=null,g=null;c.length>0;)h=a.token.findStart(c),a.log.trace("Twig.tokenize: ","Found token: ",h),h.position!==null?(h.position>0&&b.push({type:a.token.type.raw,value:c.substring(0,
h.position)}),c=c.substr(h.position+h.def.open.length),e+=h.position+h.def.open.length,g=a.token.findEnd(c,h.def,e),a.log.trace("Twig.tokenize: ","Token ends at ",g),b.push({type:h.def.type,value:c.substring(0,g).trim()}),c=c.substr(g+h.def.close.length),e+=g+h.def.close.length):(b.push({type:a.token.type.raw,value:c}),c="");return b};a.compile=function(c){for(var b=[],e=[],h=[],g=null,i=g=g=null,j=null,l=i=null,k=null,m=null;c.length>0;){g=c.shift();a.log.trace("Compiling token ",g);switch(g.type){case a.token.type.raw:e.length>
0?h.push(g):b.push(g);break;case a.token.type.logic:g=a.logic.compile.apply(this,[g]);l=g.type;k=a.logic.handler[l].open;m=a.logic.handler[l].next;a.log.trace("Twig.compile: ","Compiled logic token to ",g," next is: ",m," open is : ",k);if(k!==void 0&&!k){i=e.pop();j=a.logic.handler[i.type];if(j.next.indexOf(l)<0)throw Error(l+" not expected after a "+i.type);i.output=i.output||[];i.output=i.output.concat(h);h=[];i={type:a.token.type.logic,token:i};e.length>0?h.push(i):b.push(i)}if(m!==void 0&&m.length>
0){a.log.trace("Twig.compile: ","Pushing ",g," to logic stack.");if(e.length>0)i=e.pop(),i.output=i.output||[],i.output=i.output.concat(h),e.push(i);e.push(g)}else k!==void 0&&k&&(i={type:a.token.type.logic,token:g},e.length>0?h.push(i):b.push(i));break;case a.token.type.output:a.expression.compile.apply(this,[g]),e.length>0?h.push(g):b.push(g)}a.log.trace("Twig.compile: "," Output: ",b," Logic Stack: ",e," Pending Output: ",h)}if(e.length>0)throw g=e.pop(),Error("Unable to find an end tag for "+
g.type+", expecting one of "+g.next.join(", "));return b};a.parse=function(c,b){var e=[],h=true,g=this,b=b||{};c.forEach(function(c){a.log.debug("Twig.parse: ","Parsing token: ",c);switch(c.type){case a.token.type.raw:e.push(c.value);break;case a.token.type.logic:c=a.logic.parse.apply(g,[c.token,b,h]);if(c.chain!==void 0)h=c.chain;if(c.context!==void 0)b=c.context;c.output!==void 0&&e.push(c.output);break;case a.token.type.output:e.push(a.expression.parse.apply(g,[c.stack,b]))}});return e.join("")};
...
void 0)throw new a.Error("Unable to save template with no id");a.Templates.registry[c.id]=c};a.Templates.load=function(c){return!a.Templates.registry.hasOwnProperty(c)?null:a.Templates.registry[c]};a.Templates.loadRemote=function(c,b,e,h){var g=b.id,i=b.blocks,j=b.precompiled,l=null;g===void 0&&(g=c);if(a.Templates.registry.hasOwnProperty(g))return a.Templates.registry.hasOwnProperty(g);if(typeof XMLHttpRequest=="undefined")throw Error("Unsupported platform: Unable to do remote requests because there is no XMLHTTPRequest implementation");
var k=new XMLHttpRequest;k.onreadystatechange=function(){var b=null;if(k.readyState==4)a.log.debug("Got template ",k.responseText),j===true?(b=JSON.parse(k.responseText),l=new a.Template({data:b,blocks:i,id:g})):(l=new a.Template({data:k.responseText,blocks:i,id:g}),l.url=c),h&&h(l)};k.open("GET",c,e===true);k.send();if(e===false)return l};a.Template=function(d){var f=d.data,e=d.id,d=d.blocks,h;this.id=e;this.blocks=d||{};this.extend=null;this.tokens=b("String",f)?a.prepare.apply(this,[f]):f;this.render=
function(b,d){var f=this,e,k=d&&d.output=="blocks";this.importBlocks=function(d,e){var h=c(f.url,d),i=a.Templates.loadRemote(h,{id:h},false),e=e||false;i.render(b);Object.keys(i.blocks).forEach(function(c){if(e||f.blocks[c]===void 0)f.blocks[c]=i.blocks[c]})};e=a.parse.apply(this,[this.tokens,b]);return this.extend?(h=c(this.url,this.extend),this.parent=a.Templates.loadRemote(h,{id:h,blocks:this.blocks},false),this.parent.blocks=this.blocks,this.parent.render(b)):k===true?this.blocks:e};e!==void 0&&
...
null)throw new TypeError(" this is null or not defined");var f=Object(this),e=f.length>>>0;if({}.toString.call(a)!="[object Function]")throw new TypeError(a+" is not a function");b&&(c=b);for(d=0;d<e;){var h;d in f&&(h=f[d],a.call(c,h,d,f));d++}};if(!Object.keys)Object.keys=function(a){if(a!==Object(a))throw new TypeError("Object.keys called on non-object");var b=[],c;for(c in a)Object.prototype.hasOwnProperty.call(a,c)&&b.push(c);return b};(function(){function a(c){var a=new Date(c.getFullYear()+
1,0,4);return(a-c)/864E5<7&&(c.getDay()+6)%7<(a.getDay()+6)%7?a.getFullYear():c.getMonth()>0||c.getDate()>=4?c.getFullYear():c.getFullYear()-((c.getDay()+6)%7-c.getDate()>2?1:0)}var b="Sun,Mon,Tue,Wed,Thu,Fri,Sat".split(","),c="Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday".split(","),d="Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec".split(","),f="January,February,March,April,May,June,July,August,September,October,November,December".split(",");Date.prototype.format=function(e){if(typeof e!==
"string"||/^\s*$/.test(e))return this+"";var h=new Date(this.getFullYear(),0,1),g=this;return e.replace(/[dDjlNSwzWFmMntLoYyaABgGhHisu]/g,function(e){switch(e){case "d":return("0"+g.getDate()).replace(/^.+(..)$/,"$1");case "D":return b[g.getDay()];case "j":return g.getDate();case "l":return c[g.getDay()];case "N":return(g.getDay()+6)%7+1;case "S":return e=g.getDate(),(e=Math.abs(e)%100)%10==1&&e!=11?"st":e%10==2&&e!=12?"nd":e%10==3&&e!=13?"rd":"th";case "w":return g.getDay();case "z":return Math.ceil((h-
...
next:[a.logic.type.else_,a.logic.type.elseif,a.logic.type.endif],open:true,compile:function(b){b.stack=a.expression.compile.apply(this,[{type:a.expression.type.expression,value:b.match[1]}]).stack;delete b.match;return b},parse:function(b,c,d){var f="",e=a.expression.parse.apply(this,[b.stack,c]),d=true;e===true&&(d=false,f=a.parse.apply(this,[b.output,c]));return{chain:d,output:f}}},{type:a.logic.type.elseif,regex:/^elseif\s+([^\s].*)$/,next:[a.logic.type.else_,a.logic.type.endif],open:false,compile:function(b){b.stack=
a.expression.compile.apply(this,[{type:a.expression.type.expression,value:b.match[1]}]).stack;delete b.match;return b},parse:function(b,c,d){var f="";d&&a.expression.parse.apply(this,[b.stack,c])===true&&(d=false,f=a.parse.apply(this,[b.output,c]));return{chain:d,output:f}}},{type:a.logic.type.else_,regex:/^else$/,next:[a.logic.type.endif,a.logic.type.endfor],open:false,parse:function(b,c,d){var f="";d&&(f=a.parse.apply(this,[b.output,c]));return{chain:d,output:f}}},{type:a.logic.type.endif,regex:/^endif$/,
next:[],open:false},{type:a.logic.type.for_,regex:/^for\s+([a-zA-Z0-9_,\s]+)\s+in\s+([^\s].+)$/,next:[a.logic.type.else_,a.logic.type.endfor],open:true,compile:function(b){var c=b.match[1],d=b.match[2],f=null,f=null;b.key_var=null;b.value_var=null;if(c.indexOf(",")>=0)if(f=c.split(","),f.length===2)b.key_var=f[0].trim(),b.value_var=f[1].trim();else throw new a.Error("Invalid expression in for loop: "+c);else b.value_var=c;f=a.expression.compile.apply(this,[{type:a.expression.type.expression,value:d}]).stack;
b.expression=f;delete b.match;return b},parse:function(b,c,d){var f=a.expression.parse.apply(this,[b.expression,c]),e=[],h;f instanceof Array?(h=0,f.forEach(function(d){c[b.value_var]=d;b.key_var&&(c[b.key_var]=h);e.push(a.parse.apply(this,[b.output,c]));h+=1})):f instanceof Object&&(d=f._keys!==void 0?f._keys:Object.keys(f),d.forEach(function(d){d!=="_keys"&&f.hasOwnProperty(d)&&(c[b.value_var]=f[d],b.key_var&&(c[b.key_var]=d),e.push(a.parse.apply(this,[b.output,c])))}));d=e.length===0;return{chain:d,
output:e.join("")}}},{type:a.logic.type.endfor,regex:/^endfor$/,next:[],open:false},{type:a.logic.type.set,regex:/^set\s+([a-zA-Z0-9_,\s]+)\s*=\s*(.+)$/,next:[],open:true,compile:function(b){var c=b.match[1].trim(),d=a.expression.compile.apply(this,[{type:a.expression.type.expression,value:b.match[2]}]).stack;b.key=c;b.expression=d;delete b.match;return b},parse:function(b,c,d){var f=a.expression.parse.apply(this,[b.expression,c]);c[b.key]=f;return{chain:d,context:c}}},{type:a.logic.type.filter,regex:/^filter\s+(.+)$/,
next:[a.logic.type.endfilter],open:true,compile:function(b){var c="|"+b.match[1].trim();b.stack=a.expression.compile.apply(this,[{type:a.expression.type.expression,value:c}]).stack;delete b.match;return b},parse:function(b,c,d){var f=a.parse.apply(this,[b.output,c]),b=[{type:a.expression.type.string,value:f}].concat(b.stack),c=a.expression.parse.apply(this,[b,c]);return{chain:d,output:c}}},{type:a.logic.type.endfilter,regex:/^endfilter$/,next:[],open:false},{type:a.logic.type.block,regex:/^block\s+([a-zA-Z0-9_]+)$/,
next:[a.logic.type.endblock],open:true,compile:function(a){a.block=a.match[1].trim();delete a.match;return a},parse:function(b,c,d){var f="",e="";this.blocks[b.block]===void 0&&(f=a.expression.parse.apply(this,[{type:a.expression.type.string,value:a.parse.apply(this,[b.output,c])},c]),this.blocks[b.block]=f);this.extend===null&&(e=this.blocks[b.block]);return{chain:d,output:e}}},{type:a.logic.type.endblock,regex:/^endblock$/,next:[],open:false},{type:a.logic.type.extends_,regex:/^extends\s+(.+)$/,
...
0;)a.logic.extend(a.logic.definitions.shift());a.logic.compile=function(b){var b=b.value.trim(),b=a.logic.tokenize.apply(this,[b]),c=a.logic.handler[b.type];c.compile&&(b=c.compile.apply(this,[b]),a.log.trace("Twig.logic.compile: ","Compiled logic token to ",b));return b};a.logic.tokenize=function(b){var c={},d=null,f=null,e=null,h=null,e=e=null,b=b.trim();for(d in a.logic.handler)if(a.logic.handler.hasOwnProperty(d)){f=a.logic.handler[d].type;e=a.logic.handler[d].regex;h=[];for(e instanceof Array?
h=e:h.push(e);h.length>0;)if(e=h.shift(),e=e.exec(b.trim()),e!==null)return c.type=f,c.match=e,a.log.trace("Twig.logic.tokenize: ","Matched a ",f," regular expression of ",e),c}throw new a.Error("Unable to parse '"+b.trim()+"'");};a.logic.parse=function(b,c,d){var f="",e,c=c||{};a.log.debug("Twig.logic.parse: ","Parsing logic token ",b);e=a.logic.handler[b.type];e.parse&&(f=e.parse.apply(this,[b,c,d]));return f};return a}(Twig||{});
...
a.modifier=a.match[1];delete a.match;delete a.value;d.push(a)},parse:function(b,c,d){var f=c.pop(),d=b.params&&a.expression.parse.apply(this,[b.params,d]),f=a.test(b.filter,f,d);b.modifier=="not"?c.push(!f):c.push(f)}},{type:a.expression.type.setkey,regex:/^\:/,next:a.expression.set.expressions,compile:function(b,c,d){c=d.pop();if(c.type!==a.expression.type.string)throw new a.Error("Unexpected object key: "+c);b.key=c.value;d.push(b)},parse:a.expression.fn.parse.push},{type:a.expression.type.comma,
...
a.expression.type.parameter.start]),compile:function(a,c,d){a.value=a.value.substr(1);d.push(a)},parse:function(b,c,d){var f=c.pop(),d=b.params&&a.expression.parse.apply(this,[b.params,d]);c.push(a.filter(b.value,f,d))}},{type:a.expression.type.variable,regex:/^[a-zA-Z_][a-zA-Z0-9_]*/,next:a.expression.set.operations.concat([a.expression.type.key.period,a.expression.type.key.brackets]),compile:a.expression.fn.compile.push,parse:function(b,c,d){b=a.expression.resolve(b.value,d);c.push(b)}},{type:a.expression.type.key.period,
regex:/^\.([a-zA-Z_][a-zA-Z0-9_]*)/,next:a.expression.set.operations.concat([a.expression.type.key.period,a.expression.type.key.brackets]),compile:function(a,c,d){a.key=a.match[1];delete a.match;delete a.value;d.push(a)},parse:function(b,c){var d=b.key,f=c.pop();if(f===null||f===void 0)throw new a.Error("Can't access a key "+d+" on an undefined object.");c.push(f[d])}},{type:a.expression.type.key.brackets,regex:/^\[([^\]]*)\]/,next:a.expression.set.operations.concat([a.expression.type.key.period,
a.expression.type.key.brackets]),compile:function(b,c,d){c=b.match[1];delete b.value;delete b.match;b.stack=a.expression.compile({value:c}).stack;d.push(b)},parse:function(b,c,d){b=a.expression.parse.apply(this,[b.stack,d]);d=c.pop();if(!d.hasOwnProperty(b))throw new a.Error("Model doesn't provide the key "+b);c.push(d[b])}},{type:a.expression.type.number,regex:/^\-?\d+(\.\d+)?/,next:a.expression.set.operations,compile:function(a,c,d){a.value=Number(a.value);d.push(a)},parse:a.expression.fn.parse.push_value}];
a.expression.resolve=function(a,c){var d=c[a];return typeof d=="function"?d.apply(c,[c]):d};a.expression.handler={};a.expression.extendType=function(b){a.expression.type[b]="Twig.expression.type."+b};for(a.expression.extend=function(b){if(!b.type)throw new a.Error("Unable to extend logic definition. No type provided for "+b);a.expression.handler[b.type]=b};a.expression.definitions.length>0;)a.expression.extend(a.expression.definitions.shift());a.expression.tokenize=function(b){var c=[],d=0,f=null,
e,h,g,i,j,l=[],k;k=function(){var b=Array.prototype.slice.apply(arguments);b.pop();b.pop();a.log.trace("Twig.expression.tokenize","Matched a ",e," regular expression of ",b);if(f&&f.indexOf(e)<0)return l.push(e+" cannot follow a "+c[c.length-1].type+" at template:"+d+" near '"+b[0].substring(0,20)+"...'"),b[0];l=[];c.push({type:e,value:b[0],match:b});j=true;f=i;d+=b[0].length;return""};for(a.log.debug("Twig.expression.tokenize","Tokenizing expression ",b);b.length>0;){b=b.trim();for(e in a.expression.handler)if(a.expression.handler.hasOwnProperty(e)){i=
a.expression.handler[e].next;h=a.expression.handler[e].regex;a.log.trace("Checking type ",e," on ",b);g=h instanceof Array?h:[h];for(j=false;g.length>0;)h=g.pop(),b=b.replace(h,k);if(j)break}if(!j)if(l.length>0)throw new a.Error(l.join(" OR "));else throw new a.Error("Unable to parse '"+b+"' at template position"+d);}a.log.trace("Twig.expression.tokenize","Tokenized to ",c);return c};a.expression.compile=function(b){var c=b.value,d=a.expression.tokenize(c),f=null,e=[],h=[],g=null;a.log.trace("Twig.expression.compile: ",
"Compiling ",c);for(a.log.trace("Twig.expression.compile: ","Tokens tokenized to ",d);d.length>0;)f=d.shift(),g=a.expression.handler[f.type],g.compile&&g.compile(f,h,e);for(;h.length>0;)e.push(h.pop());a.log.trace("Twig.expression.compile: ","Stack is",e);b.stack=e;delete b.value;return b};a.expression.parse=function(b,c){var d=this;b instanceof Array||(b=[b]);var f=[],e=null;b.forEach(function(b){e=a.expression.handler[b.type];e.parse&&e.parse.apply(d,[b,f,c])});return f.pop()};return a}(Twig||{});
...
b;return c};a.expression.operator.parse=function(b,c){a.log.trace("Twig.expression.operator.parse: ","Handling ",b);var d,f;switch(b){case "+":f=parseFloat(c.pop());d=parseFloat(c.pop());c.push(d+f);break;case "-":f=parseFloat(c.pop());d=parseFloat(c.pop());c.push(d-f);break;case "*":f=parseFloat(c.pop());d=parseFloat(c.pop());c.push(d*f);break;case "/":f=parseFloat(c.pop());d=parseFloat(c.pop());c.push(d/f);break;case "%":f=parseFloat(c.pop());d=parseFloat(c.pop());c.push(d%f);break;case "~":f=c.pop().toString();
d=c.pop().toString();c.push(d+f);break;case "!":c.push(!c.pop());break;case "<":f=c.pop();d=c.pop();c.push(d<f);break;case "<=":f=c.pop();d=c.pop();c.push(d<=f);break;case ">":f=c.pop();d=c.pop();c.push(d>f);break;case ">=":f=c.pop();d=c.pop();c.push(d>=f);break;case "==":f=c.pop();d=c.pop();c.push(d==f);break;case "!=":f=c.pop();d=c.pop();c.push(d!=f);break;case "||":f=c.pop();d=c.pop();c.push(d||f);break;case "&&":f=c.pop(),d=c.pop(),c.push(d&&f)}};return a}(Twig||{});
Twig=function(a){function b(a,b){var f=Object.prototype.toString.call(b).slice(8,-1);return b!==void 0&&b!==null&&f===a}a.filters={upper:function(a){return a.toUpperCase()},lower:function(a){return a.toLowerCase()},capitalize:function(a){return a.substr(0,1).toUpperCase()+a.substr(1)},title:function(a){return a.replace(/(^|\s)([a-z])/g,function(a,c,b){return c+b.toUpperCase()})},length:function(a){if(a instanceof Array||typeof a==="string")return a.length;else if(a instanceof Object)return a._keys===
void 0?Object.keys(a).length:a._keys.length},reverse:function(a){if(b("Array",a))return a.reverse();else if(b("String",a))return a.split("").reverse().join("");else{var d=a._keys||Object.keys(a).reverse();a._keys=d;return a}},sort:function(a){if(b("Array",a))return a.sort();else if(a instanceof Object){delete a._keys;var d=Object.keys(a).sort(function(b,d){return a[b]>a[d]});a._keys=d;return a}},keys:function(a){var b=[];(a._keys||Object.keys(a)).forEach(function(f){f!=="_keys"&&a.hasOwnProperty(f)&&
b.push(f)});return b},url_encode:function(a){return encodeURIComponent(a)},join:function(a,b){var f="",e=[],h=null;b&&b[0]&&(f=b[0]);a instanceof Array?e=a:(h=a._keys||Object.keys(a),h.forEach(function(b){b!=="_keys"&&a.hasOwnProperty(b)&&e.push(a[b])}));return e.join(f)},"default":function(c,b){if(b===void 0||b.length!==1)throw new a.Error("default filter expects one argument");return c===void 0||c===null||c===""?b[0]:c},json_encode:function(a){delete a._keys;return JSON.stringify(a)},merge:function(c,
b){var f=[],e=0,h=[];c instanceof Array?b.forEach(function(a){a instanceof Array||(f={})}):f={};if(!(f instanceof Array))f._keys=[];c instanceof Array?c.forEach(function(a){f._keys&&f._keys.unshift(e);f[e]=a;e++}):(h=c._keys||Object.keys(c),h.forEach(function(a){f[a]=c[a];f._keys.push(a);a=parseInt(a,10);!isNaN(a)&&a>=e&&(e=a+1)}));b.forEach(function(a){a instanceof Array?a.forEach(function(a){f._keys&&f._keys.push(e);f[e]=a;e++}):(h=a._keys||Object.keys(a),h.forEach(function(b){f[b]||f._keys.unshift(b);
f[b]=a[b];b=parseInt(b,10);!isNaN(b)&&b>=e&&(e=b+1)}))});if(b.length===0)throw new a.Error("Filter merge expects at least one parameter");return f},date:function(a,b){return(new Date(a)).format(b[0])}};a.filter=function(b,d,f){if(!a.filters[b])throw"Unable to find filter "+b;return a.filters[b](d,f)};a.filter.extend=function(b,d){a.filters[b]=d};return a}(Twig||{});
